...
def rdm2x2_NN(state, env, sym_pos_def=False, force_cpu=False, verbosity=0, **kwargs):
    ...
    from .sliced_einsum import sliced_einsum
    ts = (C2x2c.flip_signature(), C2x2, C2x2c, C2x2.flip_signature())
    einsum_string = "abcdef,defghimn,jklabc,ghijklpq->mnpq"
    rdm=sliced_einsum(einsum_string, *ts, sliced="mnpq", checkpoint_move=True)
...
def rdm2x2_NNN(state, env, sym_pos_def=False, force_cpu=False, verbosity=0, **kwargs):
    ...
    from .sliced_einsum import sliced_einsum
    ts = (C2x2c.flip_signature(), C2x2, C2x2c.flip_signature(), C2x2)
    einsum_string = "abcdef,defghimn,ghijkl,jklabcpq->mnpq"
    rdm=sliced_einsum(einsum_string, *ts, sliced="mnpq", checkpoint_move=True)

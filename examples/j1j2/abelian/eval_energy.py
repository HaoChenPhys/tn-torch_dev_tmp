...
def main():
    ...
    def loss_c4v_fp(state, ctm_env_in, opt_context):
        ...
        # =============for debug=============
        import pickle
        d = ctm_env_out.save_to_dict()
        with open(args.out_prefix+"_env_dict", 'wb') as file:
            pickle.dump(d, file)
        # t_ctm = 0
        # import pickle
        ...
        #     return env
        # with open(args.out_prefix+"_env_dict", 'rb') as file:
        #     d = pickle.load(file)
        # env_leg = yastn.Leg(state_yastn.config, s=1, t=(0,), D=(1,))
        # ctm_env_in = EnvCTM_c4v(state_yastn, init=YASTN_ENV_INIT[ctm_args.ctm_env_init_type], leg=env_leg)
        # ctm_env_out = load_env_from_dict(ctm_env_in, d, state_yastn.config)
        # print("load env")
        # ===================================
    ...
    opt_context= dict({"ctm_args":cfg.ctm_args, "opt_args":cfg.opt_args})
    opt_context["line_search"] = None
    with torch.no_grad():
        loss, ctm_env_out, _, _, _, _= loss_fn(state, None, opt_context)
        # opt_context["loss_history"] = {'loss': [loss]}
        # obs_fn(state, ctm_env_out, opt_context)
